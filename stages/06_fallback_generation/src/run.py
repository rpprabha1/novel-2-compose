...
Agent half (writes the image-generation prompt from beat data, see
AGENT_PROMPT.md) + code half (renders the prompt, Ken Burns zoompan into
video-length footage). HYBRID per CLAUDE.md section 2. Only processes beats
...
from shared.media import FFmpegError, ken_burns_zoompan  # noqa: E402
...
def main(
    ...
    # scene_id are always authoritatively known from context anyway.
    ...
    # Persist the agent half's output for review/audit, same as every other
    # agent stage (e.g. 02's beats.json) - this was missing on the first real
    # run and only the final assets_manifest.json was written.
    ...
    output = {"run_id": run_id, "scene_id": parsed["scene_id"], "assets": assets}
    ...
    return StageResponse(
        ...
        output_manifest=["outputs/fallback_prompt.json", "outputs/assets_manifest.json"],
    )
